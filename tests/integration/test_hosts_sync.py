"""
Integration test for the managed /etc/hosts block (opt-in name resolution).

A hub started with --hosts-sync should, once its daemon is up, write a marked
block mapping its overlay address to "<hostname>.internal" — validating the
config-write → reconcile → /etc/hosts path end to end.
"""
import time
...
def test_hub_writes_managed_hosts_block(gw_image, gw_network):
    ...
    try:
        ...
        for _ in range(15):
            ...
            if "BEGIN greasewood" in block and "hubby.internal" in block:
                break
            time.sleep(2)
        assert "BEGIN greasewood" in block, f"no managed block:\n{block}"
        # the overlay address maps to the mesh name
        assert any("hubby.internal" in ln and "fd8d:" in ln
