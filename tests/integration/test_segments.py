...
Nodes peer only within a shared segment; the hub (segment:* by default) reaches
every segment. Exercises the positive (same-segment link forms), the negative
(cross-segment link never forms), and hub-reaches-all. Dedicated hub so the
...
def test_segments_partition_the_mesh(gw_image, gw_network):
    cids = []
    try:
        hub = make_hub(gw_image, gw_network, hostname="seghub")
        cids.append(hub["cid"])
        ...
        # The hub carries segment:*, so every node reaches it regardless.
        for n in (a, c, b):
