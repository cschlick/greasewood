...
def test_ipv4_underlay_mesh_converges(gw_image):
    ...
    cids = []
    try:
        anchor = make_anchor(gw_image, net, hostname="v4anchor")
        cids.append(anchor["cid"])

        # Sanity: the underlay really is IPv4-only.
        assert container_ipv4(anchor["cid"], net), "anchor got no v4 underlay address"
        assert not container_ipv6(anchor["cid"], net), \
            "network unexpectedly has IPv6 — not a v4-only underlay test"

        a = bring_up_node(gw_image, net, anchor, hostname="v4-a")
        cids.append(a["cid"])
        b = bring_up_node(gw_image, net, anchor, hostname="v4-b")
        cids.append(b["cid"])

        # The overlay is IPv6, transported over the v4 underlay: every node
        # reaches the anchor and each other on their v6 overlay addresses.
        for n in (a, b):
            assert wait_for_ping(n["cid"], anchor["overlay"], timeout=60), \
                f"{n['hostname']} can't reach the anchor overlay over the v4 underlay"
