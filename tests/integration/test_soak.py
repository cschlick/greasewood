...
every few seconds for several minutes, asserting at EVERY sample that:
  ...
  * every overlay tunnel still carries traffic (all-pairs ping from the anchor),
  * the anchor's directory still holds every node, and
...
Gated behind GW_SOAK=1 (long-running). Tunables:
  ...
  GW_SOAK_N       number of non-anchor nodes           (default 3)
...
def _max_exp(anchor_cid: str) -> str:
    """The latest credential expiry the anchor is currently serving — rises each
    time any node renews and republishes."""
    return max((r["cred"]["exp"] for r in directory_records(anchor_cid)), default="")


def test_mesh_soak(gw_image, gw_network):
    cids = []
    try:
        anchor = make_anchor(gw_image, gw_network, ttl=TTL, hostname="soakanchor")
        cids.append(anchor["cid"])
        nodes = []
        for i in range(N):
            node = bring_up_node(gw_image, gw_network, anchor, hostname=f"soak{i}")
            cids.append(node["cid"])
            nodes.append(node)

        members = [anchor] + nodes            # everyone, full mesh
        ...
        for m in members:
            ...
            assert got >= expected_peers, (
                f"{m.get('hostname', 'anchor')} only reached {got}/{expected_peers} "
                f"peers before the soak started")

        overlays = [m["overlay"] for m in nodes]  # ping targets (anchor → each node)
        start = time.time()
        first_exp = _max_exp(anchor["cid"])
        samples = 0
        timeline = []
        ...
        while time.time() - start < DURATION:
            ...
            for m in members:
                ...
                assert pc >= expected_peers, (
                    f"[t={elapsed}s] {m.get('hostname', 'anchor')} dropped to "
            ...
            for node in nodes:
                assert ping_once(anchor["cid"], node["overlay"], timeout=3), (
                    f"[t={elapsed}s] anchor → {node['hostname']} "
                    f"({node['overlay']}) ping failed mid-soak.\nTimeline:\n"
                    + "\n".join(timeline))

            # 3. Directory still complete (anchor + N nodes).
            dir_n = len(directory_records(anchor["cid"]))
            assert dir_n >= len(members), (
                f"[t={elapsed}s] anchor directory shrank to {dir_n}/{len(members)} "
                f"records.\nTimeline:\n" + "\n".join(timeline))
            ...
            line = (f"t={elapsed:>4}s  peers=ok  ping=ok  dir={dir_n}  "
                    f"worst_handshake={worst_hs}s  max_exp={_max_exp(anchor['cid'])[-9:]}")
            timeline.append(line)
            print(line, flush=True)
            samples += 1
            time.sleep(SAMPLE)

        # Renewals actually happened: the newest expiry the anchor serves must have
        # advanced past where we started (otherwise nothing renewed and we merely
        # ran shorter than one TTL).
        last_exp = _max_exp(anchor["cid"])
        assert last_exp > first_exp, (
            f"no renewal observed over {DURATION}s: max credential expiry stayed "
            f"at {first_exp} — renewals aren't propagating to the anchor.\n"
            + "\n".join(timeline))
