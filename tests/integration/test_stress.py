...
def _grow(gw_image, gw_network, gw_anchor, count, workers=MAX_WORKERS):
    """Bring up `count` nodes concurrently. Returns the list of node dicts."""
    nodes = []
    errors = []
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [
            pool.submit(bring_up_node, gw_image, gw_network, gw_anchor)
...
def _hostname(member):
    return member.get("hostname", "anchor")
...
def test_grow_mesh_to_n_nodes(gw_anchor, gw_image, gw_network):
    """
    ...
    try:
        t0 = time.time()
        nodes = _grow(gw_image, gw_network, gw_anchor, N_NODES)
        ...
        got = wait_for_directory_size(gw_anchor["cid"], expected_records, timeout=60)
...
def test_mesh_growth_in_waves(gw_anchor, gw_image, gw_network):
    """
    ...
    try:
        for target in WAVES:
            ...
            if add:
                t0 = time.time()
                nodes += _grow(gw_image, gw_network, gw_anchor, add)
