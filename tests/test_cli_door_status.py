"""
`gw status` shows the enrollment door's state (hub only):
...
def test_non_hub_has_no_door_block(tmp_path, capsys):
    # A node isn't a hub; even if a stray status file existed, don't show it.
    args = _hub(tmp_path, role="node")
