...
def test_anchor_rules_cover_underlay_control_and_door():
    # greasewood installs no packet filter of its own, so an anchor's full
    # need is checkable: the two underlay UDP ports on the physical NIC, plus
    # the overlay control plane and the door's enrollment exchange.
    ports = {(r.proto, r.port, r.iif) for r in fw.anchor_rules()}
    assert ports == {("udp", 51900, None), ("udp", 51901, None),
                     ("tcp", 51902, "gw-mesh"), ("tcp", 51903, "gw-door")}
...
def test_anchor_rules_use_the_real_mesh_interface():
    # The control-plane rule must be scoped to the actual gw-<name>
    # interface, not the stale hardcoded "gw-mesh".
