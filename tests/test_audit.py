...
def test_record_command_failure_is_error_with_stderr(caplog):
    with caplog.at_level(logging.INFO, logger="greasewood.audit"):
        audit.record_command(["ip", "-6", "route", "del", "x"], 2, 3, "", "no such device")
    rec = caplog.records[-1]
    assert rec.levelno == logging.ERROR
    assert "rc=2" in rec.message and 'stderr="no such device"' in rec.message
...
def test_context_tags_commands_and_resets(caplog):
    with caplog.at_level(logging.INFO, logger="greasewood.audit"):
        ...
        with audit.context("reconcile: +peer db01"):
            ...
            audit.record_command(["wg", "show"], 0, 1)
...
def test_wg_run_emits_an_audit_record(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="greasewood.audit"):
        wg._run("ip", "-6", "addr", "show")
    assert any('argv="ip -6 addr show"' in r.message for r in caplog.records)
...
def test_attach_file_writes_rotating_0600(tmp_path):
    ...
    try:
        ...
        audit.record_command(["ip", "link", "show"], 0, 1)
        for hh in audit.log.handlers:
            hh.flush()
        text = p.read_text()
        assert "ts=" in text and 'argv="ip link show"' in text
