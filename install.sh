...
install_deps_linux() {
    local need_wg=1 need_ip=1 need_venv=1 pyver
    pyver=$("$PY" -c 'import sys; print("%d.%d" % sys.version_info[:2])')
    command -v wg >/dev/null 2>&1 && need_wg=0
    command -v ip >/dev/null 2>&1 && need_ip=0            # greasewood shells out to `ip`
    ...
    [ "$need_wg" -eq 0 ] && [ "$need_ip" -eq 0 ] && [ "$need_venv" -eq 0 ] \
        && { say "runtime deps present (wg, ip, venv)"; return; }

    say "installing runtime deps (wireguard-tools, iproute2, nftables, python venv)"
    if command -v apt-get >/dev/null 2>&1; then
        apt-get update -qq
        apt-get install -y wireguard-tools iproute2 nftables python3-venv
        ...
        dnf install -y wireguard-tools iproute nftables
    elif command -v yum >/dev/null 2>&1; then
        yum install -y wireguard-tools iproute nftables
    elif command -v pacman >/dev/null 2>&1; then
        pacman -Sy --noconfirm wireguard-tools iproute2 nftables
    elif command -v zypper >/dev/null 2>&1; then
        zypper --non-interactive install wireguard-tools iproute2 nftables
    elif command -v apk >/dev/null 2>&1; then
        # Alpine: python3 ships a working venv/ensurepip (no split package like
        # Debian's python3-venv), so just the tools. iproute2 gives the real `ip`
        # (busybox's applet is a subset). Note: install.sh needs bash to run at
        # all (Alpine base has none) — `apk add bash` first.
        apk add --no-cache wireguard-tools iproute2 nftables
    else
        warn "no known package manager — install 'wireguard-tools', 'iproute2'"
        warn "(and a python3 with venv) yourself, then re-run."
    fi
    command -v wg >/dev/null 2>&1 || die "'wg' still not found after dep install — install wireguard-tools manually"
    command -v ip >/dev/null 2>&1 || die "'ip' still not found after dep install — install iproute2 manually"
