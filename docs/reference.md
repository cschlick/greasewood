...
| `explain [A [B]]`  | no    | The **story** of a node (or a pair): one chronological timeline merged from this machine's audit events, the mesh's replicated membership decisions (revokes/leaves/role changes — visible even when another holder decided them), credential renewals, and peers' endpoint testimony, ending in a current-state verdict. Departed nodes still resolve by name. `--since 24h` sets the window. First stop when something looks wrong; `narrate` is the command-level microscope, `diagnose` the live probe. |
...
| `revoke <node>`    | no    | Revoke a node (on any anchor-file holder; the decision replicates — denies renew/publish, evicts it, frees its hostname). `<node>` = hostname, `<host>.<mesh_domain>` mesh name, or 64-char id_pub hex. |
| `set-roles <node> <r>` | no | Change a node's roles (on any holder; a replicated statement, effective at its next renewal). |
| `policy`           | show: no · apply: sudo | The mesh's grant table (roles → roles : ports; derives which tunnels exist). `show` renders the active policy on any node; `apply` (anchor) validates `grants.toml`, previews the tunnel delta, signs with the CA key, publishes. See [Roles & the grant table](access-control.md). |
| `set-caps <node> <caps>` | no | Change a node's full tag set (on any holder; a replicated statement, effective at its next renewal). |
...
| `renew-all`        | no    | On any holder: request a fleet-wide renewal (advertise `renew_after=now`, gossiped between holders; cooperating nodes renew, jittered so the holders' rate stays ~constant with mesh size). |
| `upgrade-all`      | yes   | On any holder: announce a release for the fleet — one CA-signed statement pinning the version **and the sha256 of its published tarball**, gossiped like every statement. Nodes with `auto_upgrade = true` (under `[network]`, default **off**) download that exact artifact, verify the hash, reinstall and restart after a short jitter; everyone else shows the announcement in `gw watch`. Defaults to the version this holder runs — upgrade here first (the canary), then announce. See [Operations](operations.md#sop-upgrade-greasewood-without-dropping-the-mesh). |
| `anchor-backup`       | no    | On any holder: write one passphrase-encrypted archive of the anchor file (CA + door keys), the replicated statements, the revoke list, and the holder's identity. Store it offline. Membership itself needs no backup — it lives in every node's replicated record. |
...
| `leave`            | yes   | Voluntarily depart the mesh: any holder forgets this node (tombstoned — hostname frees immediately, records die fleet-wide on the next sync), then the local daemon/interface tear down. Keys/config KEPT (`purge` erases); re-enrolling later takes a fresh invite. |
| `anchor <action>`  | status: no · rest: sudo | The **anchor file** — the mesh's portable authority (see [The anchor file](anchor.md)). `status` (holder state + the fleet's holders), `init` (fold a legacy anchor's keys into the file), `offer <node>` (mint a single-use 15-min offer sealed to that node's WireGuard key, served over the mesh), `adopt` (collect the offer — or install an exported file — and become a holder), `export` (manual-transfer fallback), `drop` (shed the roles, delete this copy — the cooperative de-anchor). |
| `service enable\|disable` | yes | Install/remove this membership's daemon service (systemd/OpenRC/launchd) — the adoption path for a config that arrived without one. |
| `upgrade`          | yes   | Reinstall greasewood in place (clean pipx uninstall+install; fetches BEFORE uninstalling so a network failure changes nothing) and restart the daemon. Default source: the latest PyPI release; `--from github` installs main. |
